...
struct PeerConnection {
  ...
  uint16_t last_audio_seq;
  uint8_t audio_seq_initialized;
};
...
int peer_connection_send_audio(PeerConnection* pc, const uint8_t* buf, size_t len) {
  if (pc->state != PEER_CONNECTION_COMPLETED || pc->dtls_srtp.srtp_out == NULL) {
    static uint32_t blocked_count = 0;
    blocked_count++;
    if (blocked_count == 1 || blocked_count % 100 == 0) {
      LOGW("audio blocked state=%s srtp_out=%p count=%" PRIu32,
           peer_connection_state_to_string(pc->state),
      ...
      while ((pc->agent_ret = agent_recv(&pc->agent, pc->agent_buf, sizeof(pc->agent_buf))) > 0) {
        ...
        if (rtcp_probe(pc->agent_buf, pc->agent_ret)) {
          ...
          if (ssrc == pc->remote_assrc) {
            RtpHeader* rtp_header = (RtpHeader*)pc->agent_buf;
            uint16_t seq = ntohs(rtp_header->seq_number);
            if (pc->audio_seq_initialized) {
              uint16_t expected_seq = pc->last_audio_seq + 1;
              int16_t seq_delta = (int16_t)(seq - expected_seq);
              ...
              if (seq_delta > 0) {
                if (pc->config.onaudiotrack) {
                  int concealed = seq_delta;
                  if (concealed > 3) {
                    concealed = 3;
                  }
                  for (int i = 0; i < concealed; ++i) {
                    pc->config.onaudiotrack(NULL, 0, pc->config.user_data);
                  }
                }
              }
            }
            pc->last_audio_seq = seq;
            pc->audio_seq_initialized = 1;
...
void peer_connection_set_remote_description(PeerConnection* pc, const char* sdp, SdpType type) {
  ...
  while ((line = strstr(start, "\r\n"))) {
    ...
    if (strstr(buf, "a=fingerprint:sha-256 ") == buf) {
      ...
      if (fingerprint != NULL) {
        ...
        if (fingerprint_len >= DTLS_SRTP_FINGERPRINT_LENGTH) {
          fingerprint_len = DTLS_SRTP_FINGERPRINT_LENGTH - 1;
        }

        memcpy(pc->dtls_srtp.remote_fingerprint, fingerprint, fingerprint_len);
