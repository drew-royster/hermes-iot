...
int udp_socket_open(UdpSocket* udp_socket, int family, int port) {
  ...
  do {
    ...
    if (setsockopt(udp_socket->fd, SOL_SOCKET, SO_SNDBUF, &send_buf_size, sizeof(send_buf_size)) < 0) {
      LOGW("SO_SNDBUF failed. ignore");
    }
...
int udp_socket_sendto(UdpSocket* udp_socket, Address* addr, const uint8_t* buf, int len) {
  ...
  socklen_t sock_len;
  int ret = -1;
  static uint32_t sendto_failure_count = 0;
  ...
  if ((ret = sendto(udp_socket->fd, buf, len, 0, sa, sock_len)) < 0) {
    ++sendto_failure_count;
    if (sendto_failure_count == 1 || (sendto_failure_count % 50) == 0) {
      LOGW("Failed to sendto: %s count=%" PRIu32, strerror(errno), sendto_failure_count);
    }
    return -1;
  }
