...
static int rtp_payload_bounds(uint8_t* buf, size_t size, uint8_t** payload, size_t* payload_size) {
  if (size < sizeof(RtpHeader)) {
    return -1;
  }

  RtpHeader* rtp_header = (RtpHeader*)buf;
  size_t header_size = sizeof(RtpHeader) + (rtp_header->csrccount * sizeof(uint32_t));
  if (size < header_size) {
    return -1;
  }
  ...
  if (rtp_header->extension) {
    ...
    memcpy(&extension_words, buf + header_size + 2, sizeof(extension_words));
    header_size += 4 + (ntohs(extension_words) * sizeof(uint32_t));
    if (size < header_size) {
      return -1;
    }
  }
  ...
  if (rtp_header->padding) {
    ...
    uint8_t padding_size = buf[size - 1];
...
static int rtp_decode_generic(RtpDecoder* rtp_decoder, uint8_t* buf, size_t size) {
  ...
  if (rtp_payload_bounds(buf, size, &payload, &payload_size) != 0) {
    return -1;
  }
