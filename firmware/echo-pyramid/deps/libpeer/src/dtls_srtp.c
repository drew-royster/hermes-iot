...
static int dtls_srtp_key_derivation(DtlsSrtp* dtls_srtp, const unsigned char* master_secret, size_t secret_len, const unsigned char* randbytes, size_t randbytes_len, mbedtls_tls_prf_types tls_prf_type) {
  ...
  LOGI("DTLS-SRTP key derivation start role=%d", dtls_srtp->role);
...
static void dtls_srtp_key_derivation_cb(void* context,
  ...
  LOGI("DTLS-SRTP export keys callback (mbedtls 2.x)");
  ...
  LOGI("DTLS-SRTP export keys callback");
...
int dtls_srtp_handshake(DtlsSrtp* dtls_srtp, Address* addr) {
  ...
  LOGI("DTLS-SRTP handshake result ret=%d profile=%d state=%d srtp_in=%p srtp_out=%p",
       ret,
       (int)dtls_srtp_negotiation_result.MBEDTLS_PRIVATE(chosen_dtls_srtp_profile),
       dtls_srtp->state,
       dtls_srtp->srtp_in,
       dtls_srtp->srtp_out);
...
void dtls_srtp_decrypt_rtp_packet(DtlsSrtp* dtls_srtp, uint8_t* packet, int* bytes) {
  if (dtls_srtp == NULL || dtls_srtp->srtp_in == NULL || bytes == NULL || *bytes <= 0) {
    if (bytes != NULL) {
      *bytes = 0;
    }
    return;
  ...
  if (srtp_unprotect(dtls_srtp->srtp_in, packet, bytes) != srtp_err_status_ok) {
    *bytes = 0;
  }
}
...
void dtls_srtp_decrypt_rtcp_packet(DtlsSrtp* dtls_srtp, uint8_t* packet, int* bytes) {
  ...
  if (srtp_unprotect_rtcp(dtls_srtp->srtp_in, packet, bytes) != srtp_err_status_ok) {
    *bytes = 0;
  }
}

void dtls_srtp_encrypt_rtp_packet(DtlsSrtp* dtls_srtp, uint8_t* packet, int* bytes) {
  if (dtls_srtp == NULL || dtls_srtp->srtp_out == NULL || bytes == NULL || *bytes <= 0) {
    if (bytes != NULL) {
      *bytes = 0;
    }
    return;
